...
class TestQuestionDataProcessor:
    ...
    def test_preprocess_data(self, processor):
        ...
        assert len(processed_df) == len(processed_df['cleaned_question'].unique())  # No duplicates
    ...
    def test_process_datasets(self, processor):
        ...
        assert all(key in processed_data for key in ['questions', 'embeddings', 'faiss_index', 'metadata'])
    ...
    def test_dimension_handling(self, processor):
        ...
        tokens_tensor = processor.encoding_manager.to_tensor([tokens], "long").to(processor.encoding_manager.device)
